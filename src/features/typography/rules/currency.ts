 ...
 *
 * currency-rubles — «90руб.», «90 руб.», «5000 рублей», «100 р.» →
 * «90\u00A0₽», «5000\u00A0₽», «100\u00A0₽». Только после числа: слово
 * «руб.» само по себе не трогается; суммы с копейками («100 р. 20 коп.»)
 * не меняются.
 *
 * Точка после «руб.»/«р.» неоднозначна: это может быть и точка сокращения
 ...
 *  - «руб.» перед строчной буквой/цифрой/пунктуацией → точка сокращения
 *    съедается («100 ₽ или», «5000 ₽,»).
 */
const R = "[рР][уУ][бБ]";
...
/**
 * Символы, после которых точка сокращения точно не конец предложения:
 * строчные буквы, цифры, открывающие скобки/кавычки, тире и знаки
 * препинания («5000 руб., 200 руб.;» → «5000 ₽, 200 ₽;»).
 */
const PUNCT_CLASS = "[а-яёa-z0-9(«\\u2014\\u2013,;:!?)\\]»\\u201D]";

/** Защита копеек: «100 р. 20 коп.» и «100 руб. 20 коп.» не меняются. */
const KOPEKS_GUARD = "(?![ \\t\\u00A0]*\\d+[ \\t\\u00A0]*[кК]оп)";

export const currencyRublesRule: TypographyRule = {
  id: "currency-rubles",
  name: "Знак рубля ₽",
  description:
    "Заменяет текстовые обозначения рубля после числа на знак ₽ с неразрывным пробелом («90руб.» → «90\u00A0₽», «5000 рублей» → «5000\u00A0₽», «100 р.» → «100\u00A0₽»); суммы с копейками («100 р. 20 коп.») не трогаются.",
  category: "units",
  ...
  apply(text, ctx) {
    ...
    // Точка перед строчной буквой/цифрой/пунктуацией — точка сокращения,
    // убираем её: «100 ₽ или», «5000 ₽,».
    result = applyRegex(
      ctx,
      currencyRublesRule,
      result,
      new RegExp(
        "(\\d+)[ \\t\\u00A0]*" + RUBLE_WORD + "\\." + KOPEKS_GUARD + "(?=[ \\t\\u00A0]*" + PUNCT_CLASS + ")",
        "gu",
      ),
      (m) => m[1] + "\u00A0\u20BD",
    );
    // Короткая форма: «100 р.», «100р.» → «100 ₽». После числа «р.» —
    // это рубли; исключение — копейки («100 р. 20 коп.» не меняется).
    // Точка после «р.» обрабатывается так же, как у «руб.».
    result = applyRegex(
      ctx,
      currencyRublesRule,
      result,
      new RegExp("(\\d+)[ \\t\\u00A0]*р\\." + KOPEKS_GUARD + "(?=[\\n\\r]|$)", "gu"),
      (m) => m[1] + "\u00A0\u20BD.",
    );
    result = applyRegex(
      ctx,
      currencyRublesRule,
      result,
      new RegExp("(\\d+)[ \\t\\u00A0]*р\\." + KOPEKS_GUARD + "(?=[ \\t\\u00A0]+\\p{Lu})", "gu"),
      (m) => m[1] + "\u00A0\u20BD.",
    );
    result = applyRegex(
      ctx,
...
export const currencySignGlueRule: TypographyRule = {
  ...
  apply(text, ctx) {
    ...
    // «20000$», «100 $», «100₽» → «20 000 $», «100 $», «100 ₽» (с
    // неразрывным пробелом): знак после числа.
    result = applyRegex(
      ctx,
      currencySignGlueRule,
      result,
