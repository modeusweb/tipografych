 ...
 * Валюта и число.
 *
 * Знак ПОСЛЕ числа отделяется неразрывным пробелом — русская традиция:
 * «20000$» → «20 000 $», «100 $» → «100 $». Знак ПЕРЕД числом пишется
 * слитно, а абсурдный пробел между ними убирается: «$ 100» → «$100».
 *
 * Выполняется ДО «currency-spacing» (пресет «Издательская»): там та же
 * отбивка задаётся отдельным правилом, а «currency-sign-position»
 * переносит знак за число — результат согласован.
 */
export const currencySignGlueRule: TypographyRule = {
  id: "currency-sign-glue",
  name: "Валюты: знак и число",
  description:
    "Знак валюты после числа отделяется неразрывным пробелом («20000$» → «20 000 $»); знак перед числом пишется слитно («$ 100» → «$100»).",
  category: "units",
  enabledByDefault: true,
  apply(text, ctx) {
    // «$ 100» → «$100»: знак перед числом — без пробела.
    let result = applyRegex(
      ctx,
    ...
    // «20000$», «100 $» → «20 000 $», «100 $» (с неразрывным пробелом):
    // знак после числа.
    result = applyRegex(
      ctx,
      currencySignGlueRule,
