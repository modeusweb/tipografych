...
export const timeFormatRule: TypographyRule = {
  ...
  name: "Время",
  description:
    "Склеивает время в формате ЧЧ:ММ («15 : 30» → «15:30») и распознаёт запись через дефис с нулевыми минутами («9-00» → «9:00»): внутри времени пробелов быть не должно.",
  category: "punctuation",
  enabledByDefault: true,
  apply(text, ctx) {
    // «9-00» → «9:00»: нули после дефиса — это минуты, а не конец
    // диапазона «от 9 до нуля». Диапазоны («13-14») не трогаются —
    // вторая часть не начинается с нуля. Идемпотентно: после замены
    // дефиса нет.
    const result = applyRegex(
      ctx,
      timeFormatRule,
      text,
      /\b([01]?\d|2[0-3])[-\u2013\u2014]00\b(?!\d)/g,
      (m) => `${m[1]}:00`,
    );
    return applyRegex(
      ctx,
