...
const SECTIONS: Array<{ title: string; items: string[] }> = [
  {
    ...
    items: [
      "Прямые и английские кавычки — на «ёлочки», вложенные — на „лапки“, третий уровень — на одинарные ‚марчуковские‘ (стек вложенности: кавычка после тире тоже открывает вложенную цитату: «слоган - \" Качество … ! \" - должен работать»); точка после закрывающей кавычки убирается, если цитата уже завершена знаком («…дней…». → «…дней…»).",
      "Дефис с пробелами — на длинное тире (но «IT - индустрия» → «IT-индустрия»), диапазоны чисел — на короткое тире, дефис перед числом — на типографский минус −; разорванное дефисное слово («кто - то», «как- либо») склеивается обратно; «Дефис-это» → «Дефис — это», «привет,-» → «привет, —» и реплика «- Привет!» → «— Привет!» (в Markdown списки не трогаются).",
      ...
      "Знаки охраны прав: «(c)», «(r)», «(tm)» → ©, ®, ™; дроби «1/2» и «1 / 2» → ½; «5х5» и «3 x 10» → «5×5», «3×10»; степень «10^8» → «10⁸»; «5 +- 3» и «+/−0.05» → «5 ± 3», «±0.05»; «x >= y» и «(x + y) <= z» → «x ≥ y», «(x + y) ≤ z»; стрелки «->», «<-», «=>», «<=>» → →, ←, ⇒, ⇔.",
      ...
      "Запись температуры: «20 градусов Цельсия» → «20 °C», «(+25 С)» и опечатка «+25oС» → «(+25 °C)».",
      "Валюты: «90руб.» и «5000 рублей» → «90 ₽» и «5000 ₽»; знак валюты после числа отделяется неразрывным пробелом («20000$» → «20 000 $»), перед числом — слитно («$ 100» → «$100»; отбивка и перенос знака за число — в пресете «Издательская»); «100долл.» → «100 долл.».",
